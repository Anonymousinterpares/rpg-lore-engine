...
import { AbilityScore } from '../schemas/BaseSchemas';

export interface MulticlassPrereq {
    class: string;
    ability: AbilityScore | AbilityScore[]; // Single or multiple (e.g., Paladin needs STR and CHA)
}

export class MulticlassingEngine {
    private static readonly PREREQS: MulticlassPrereq[] = [
     ...
     */
    public static canMulticlass(pc: PlayerCharacter, targetClass: string): { success: boolean; message: string } {
    ...
    private static checkPrereq(pc: PlayerCharacter, prereq: MulticlassPrereq): boolean {
    ...
    private static getPrereqLabel(prereq: MulticlassPrereq): string {
        if (Array.isArray(prereq.ability)) {
