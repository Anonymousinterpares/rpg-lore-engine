...
export class ShopEngine {
     ...
     */
    static negotiate(pc, charismaBonus, npc) {
        if (!npc.isMerchant || !npc.shopState || !npc.shopState.isOpen)
            return { success: false, message: "Shop is closed." };
        const standing = npc.relationship.standing;
        if (standing <= -25)
            return { success: false, message: "The merchant refuses to speak with you." };
        ...
        if (roll >= dc) {
            ...
            if (npc.relationship.standing <= -25)
     ...
     */
    static buyItem(pc, item, npc) {
        if (!npc.isMerchant || !npc.shopState || !npc.shopState.isOpen)
            return "Shop is closed.";
        if (npc.relationship.standing <= -25)
            return "The merchant refuses to do business with you.";
     ...
     * Processes a sale.
     */
    static sellItem(pc, inventoryItemId, npc, fullItemData) {
        if (!npc.isMerchant || !npc.shopState || !npc.shopState.isOpen)
            return "Shop is closed.";
        if (npc.relationship.standing <= -25)
            return "The merchant refuses to do business with you.";
    ...
    static updateRelationship(npc, event, delta) {
    ...
    static getStandingModifier(standing) {
        if (standing >= 75)
            return 0.8; // Exalted
        if (standing >= 25)
            return 0.9; // Friendly
        if (standing <= -75)
            return 1.5; // Hostile (if they even trade)
        if (standing <= -25)
            return 1.2; // Unfriendly
        return 1.0; // Neutral
    }
}
