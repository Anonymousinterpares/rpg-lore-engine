...
export class QuestEngine {
     ...
     */
    public static updateObjective(quest: Quest, objectiveId: string, progress: number): string {
        const objective = quest.objectives.find(o => o.id === objectiveId);
        if (!objective) return `Objective ${objectiveId} not found in quest ${quest.title}.`;
     ...
     */
    public static completeQuest(state: GameState, questId: string): string {
        const quest = state.activeQuests?.find(q => q.id === questId);
        if (!quest) return `Quest ${questId} not found in active quests.`;
        if (quest.status !== 'ACTIVE') return `Quest ${quest.title} is already ${quest.status}.`;
        ...
        if (quest.rewards) {
            ...
            if (quest.rewards.xp > 0) {
                ...
                if (leveledUp) rewardMsg += ` Level up available!`;
            }
