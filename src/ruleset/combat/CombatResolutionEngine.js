...
export class CombatResolutionEngine {
     ...
     */
    static resolveAttack(attacker, target, attackBonus, damageFormula, statMod) {
        const d20 = Dice.d20();
        const isCrit = d20 === 20;
     ...
     */
    static resolveSpell(caster, target, spell, spellAttackBonus, spellSaveDC) {
        let type = 'EFFECT';
        let damage = 0;
        let heal = 0;
        let message = '';
        ...
        // 1. Resolve Hit/Save
        if (spell.save) {
            const saveAbility = spell.save.ability;
     ...
     */
    static applyHealing(target, amount) {
        target.hp.current += amount;
