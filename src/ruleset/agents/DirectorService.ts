...
import { DirectorDirectiveSchema, DirectorDirective } from './ICPSchemas';
...
export class DirectorService {
     ...
     */
    public static async evaluatePacing(state: GameState): Promise<DirectorDirective | null> {
        // Only run every few turns to save tokens, or if something significant changed
        if (state.conversationHistory.length % 5 !== 0) return null;
