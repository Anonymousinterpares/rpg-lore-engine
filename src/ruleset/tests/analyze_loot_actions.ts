...
const CUSTOM_WEAPON_MAP: Record<string, string> = {
    'Fork': 'Trident',
    'Sword': 'Longsword',
...
interface AnalysisResult {
    weaponActionMap: Record<string, string>;     // action name -> item file name
    naturalAttacks: string[];                     // confirmed natural attacks
    unknownActions: string[];                     // need manual review
    monsterActionDetails: Record<string, string[]>; // monster -> actions
}
...
function loadWeaponNames(): Map<string, string> {
...
function analyzeMonsters(weaponMap: Map<string, string>): AnalysisResult {
    ...
    for (const file of files) {
        ...
        if (content.actions && Array.isArray(content.actions)) {
            for (const action of content.actions) {
                ...
                else if (NATURAL_ATTACK_PATTERNS.some(p =>
                    baseActionName.toLowerCase().includes(p.toLowerCase())
                )) {
                    result.naturalAttacks.push(actionName);
                }
...
function main() {
    ...
    fs.writeFileSync(unknownPath,
        `=== UNKNOWN ACTIONS - MANUAL REVIEW REQUIRED ===\n` +
