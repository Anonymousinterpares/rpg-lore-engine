import { CharacterCreationEngine, CreationRequest } from '../combat/CharacterCreationEngine';
...
function runCreationSimulation() {
    ...
    try {
        ...
        // Verification checks
        if (hero.stats['DEX'] !== 16) console.error('FAIL: Racial DEX bonus not applied');
        if (hero.hp.max !== 6 + 1) console.error(`FAIL: HP mismatch. Got ${hero.hp.max}, expected 7`);
        if (!hero.skillProficiencies.includes('Religion')) console.error('FAIL: Background skill Religion missing');
        if (hero.biography.backgroundId !== 'acolyte') console.error('FAIL: Background identity missing');
