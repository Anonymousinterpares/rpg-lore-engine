...
function ingestMonsters() {
    ...
    raw.forEach((m: any) => {
        ...
        const monster = {
            ...
            stats: stats,
            traits: (m.special_abilities || []).map((a: any) => ({
                name: a.name,
                description: a.desc
            })),
...
function ingestItems() {
    ...
    raw.forEach((item: any) => {
        const cost: any = { cp: 0, sp: 0, ep: 0, gp: 0, pp: 0 };
        if (item.cost) {
        ...
        if (mapped.type === 'Weapon' && item.damage) {
            ...
            mapped.properties = (item.properties || []).map((p: any) => p.name);
        }
...
function ingestRaces() {
    ...
    rawRaces.forEach((r: any) => {
        const asi: any = {};
        (r.ability_bonuses || []).forEach((b: any) => {
            asi[b.name] = b.bonus;
        });
        ...
        const mapped: any = {
            ...
            traits: (r.traits || []).map((t: any) => ({ name: t.name })),
    ...
    rawSubraces.forEach((sr: any) => {
        const asi: any = {};
        (sr.ability_bonuses || []).forEach((b: any) => {
            asi[b.name] = b.bonus;
        });
        ...
        const mapped = {
            ...
            size: 'Medium', // Default
            abilityScoreIncreases: asi,
            traits: (sr.racial_traits || []).map((t: any) => ({ name: t.name })),
            languages: (sr.languages || []).map((l: any) => l.name)
        };
...
function ingestClasses() {
    ...
    rawClasses.forEach((c: any) => {
        const mapped: any = {
            ...
            savingThrowProficiencies: (c.saving_throws || []).map((s: any) => s.name),
            armorProficiencies: (c.proficiencies || [])
                .filter((p: any) => p.name.toLowerCase().includes('armor') || p.name.toLowerCase().includes('shield'))
                .map((p: any) => p.name),
            weaponProficiencies: (c.proficiencies || [])
                .filter((p: any) => p.name.toLowerCase().includes('weapon') || p.name.toLowerCase().includes('sword') || p.name.toLowerCase().includes('bow'))
                .map((p: any) => p.name),
            skillChoices: {
                count: c.proficiency_choices ? c.proficiency_choices[0].choose : 0,
                options: c.proficiency_choices ? c.proficiency_choices[0].from.map((f: any) => f.name) : []
            }
        };
