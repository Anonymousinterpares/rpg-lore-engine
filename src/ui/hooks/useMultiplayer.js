...
export const useMultiplayer = (lobbyUrl = 'http://localhost:4000') => {
    ...
    const hostSession = useCallback(async (name, playerId) => {
        try {
    ...
    const joinSession = useCallback(async (sessionId, playerName, characterId) => {
        try {
            const newClient = new MultiplayerClient(lobbyUrl);
