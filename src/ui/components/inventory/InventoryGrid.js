...
const InventoryGrid = ({ items, gold, onItemClick, className = '' }) => {
    ...
    return (_jsxs("div", { className: `${styles.container} ${glassStyles.glassPanel} ${className}`, children: [_jsxs("div", { className: styles.header, children: [_jsx("h3", { className: styles.title, children: "Inventory" }), _jsxs("div", { className: styles.gold, children: [_jsx(Coins, { size: 14, className: styles.goldIcon }), _jsxs("span", { children: [gold.gp, "g ", gold.sp, "s ", gold.cp, "c"] })] })] }), _jsxs("div", { className: styles.grid, children: [items.map((item) => (_jsxs("div", { className: `${styles.itemSlot} ${item.equipped ? styles.equipped : ''}`, onClick: () => onItemClick?.(item), title: `${item.name} (${item.type})`, children: [_jsx("div", { className: styles.iconWrapper, children: getItemIcon(item.type) }), item.quantity > 1 && _jsx("span", { className: styles.quantity, children: item.quantity }), item.equipped && _jsx("div", { className: styles.equippedBadge, children: "E" })] }, item.id))), [...Array(Math.max(0, 20 - items.length))].map((_, i) => (_jsx("div", { className: styles.emptySlot }, `empty-${i}`)))] })] }));
};
