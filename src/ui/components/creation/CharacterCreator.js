...
const CharacterCreator = ({ onComplete, onCancel }) => {
    ...
    const renderStepContent = () => {
        switch (step) {
            ...
            case 1: // Race
                return (_jsx("div", { className: styles.gridContainer, children: races.map(r => (_jsxs("div", { className: `${styles.card} ${selectedRace?.name === r.name ? styles.selected : ''}`, onClick: () => setSelectedRace(r), children: [_jsx("h4", { children: r.name }), _jsxs("p", { className: styles.smallInfo, children: [_jsx("strong", { children: "Speed:" }), " ", r.speed, "ft | ", _jsx("strong", { children: "Size:" }), " ", r.size] }), _jsx("p", { className: styles.smallInfo, children: r.traits.map(t => t.name).join(', ') })] }, r.name))) }));
            case 2: // Class
                return (_jsx("div", { className: styles.gridContainer, children: classes.map(c => (_jsxs("div", { className: `${styles.card} ${selectedClass?.name === c.name ? styles.selected : ''}`, onClick: () => setSelectedClass(c), children: [_jsx("h4", { children: c.name }), _jsxs("p", { className: styles.smallInfo, children: [_jsx("strong", { children: "Hit Die:" }), " ", c.hitDie, " | ", _jsx("strong", { children: "Primary:" }), " ", c.primaryAbility.join(', ')] })] }, c.name))) }));
            case 3: // Background
                return (_jsx("div", { className: styles.gridContainer, children: backgrounds.map(b => (_jsxs("div", { className: `${styles.card} ${selectedBackground?.name === b.name ? styles.selected : ''}`, onClick: () => setSelectedBackground(b), children: [_jsx("h4", { children: b.name }), _jsx("p", { className: styles.smallInfo, children: b.description })] }, b.name))) }));
            case 4: // Abilities
