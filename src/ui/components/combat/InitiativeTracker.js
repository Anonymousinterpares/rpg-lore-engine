...
const InitiativeTracker = ({ combatants, currentTurnId, selectedTargetId, onSelectTarget, className = '' }) => {
    return (_jsx("div", { className: `${styles.container} ${className}`, children: _jsx("div", { className: styles.list, children: combatants.map((c) => {
                ...
                return (_jsxs("div", { className: `${styles.combatant} ${isActive ? styles.active : ''} ${c.isPlayer ? styles.player : styles.enemy} ${isSelected ? styles.selected : ''}`, onClick: () => !c.isPlayer && onSelectTarget?.(c.id), children: [isActive && _jsx(ChevronRight, { className: styles.indicator, size: 14 }), _jsxs("div", { className: styles.info, children: [_jsx("span", { className: styles.name, children: c.name }), _jsxs("span", { className: styles.init, children: ["(", c.initiative, ")"] })] }), _jsxs("div", { className: styles.hpContainer, children: [_jsx("div", { className: styles.hpBar, style: { width: `${hpPercent}%` } }), _jsxs("span", { className: styles.hpText, children: [Math.floor(c.hp.current), "/", c.hp.max] })] }), isSelected && _jsx("div", { className: styles.targetLabel, children: "TARGET" })] }, c.id));
            }) }) }));
};
